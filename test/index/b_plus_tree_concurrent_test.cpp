...
TEST(BPlusTreeConcurrentTest, InsertTest1) {
  ...
  LaunchParallelTest(10, InsertHelper, std::ref(tree), keys);
...
TEST(BPlusTreeConcurrentTest, InsertTest2) {
  ...
  LaunchParallelTest(10, InsertHelperSplit, std::ref(tree), keys, 2);
...
TEST(BPlusTreeConcurrentTest, DeleteTest3) {
  ...
  //LOG_DEBUG("tree: %s", tree.ToString(true).c_str());

  std::vector<int64_t> remove_keys;
  for (int64_t key = scale_factor - 1; key > 0; key--) {
    remove_keys.push_back(key);
  }
  LaunchParallelTest(5, DeleteHelper, std::ref(tree), remove_keys);
